...
public class InitFbyjazDataTest {
	...
	public static void initFinishedData() throws BusinessException {
		...
		//判断 发包移交安装日期不空
		String sqlwhere1 = "where  YiJiaoAnZhuangRiQi is not null and baozhuangbianhao > '2015000084' and baozhuangbianhao <'2015001624' group by  GCMC having COUNT(gcmc)>0";
		List<FbyjazVO> list1 = (List<FbyjazVO>) NCLocator
				.getInstance()
				.lookup(IUAPQueryBS.class)
				.executeQuery(
						"select GCMC pk_jobbasfil, min(GongChengXingZhi) gcxz, min(HeTongHao) hth, min(SheJiTuHao) sjth,min (YiJiaoAnZhuangRiQi) yjazrq,min( BeiZhu) remark from bz "
						+ sqlwhere1,
						new BeanListProcessor(FbyjazVO.class));
		...
		if (list1 != null) {
			for (FbyjazVO Kcpg1 : list1) {
				//设置制单人 
				Kcpg1.setVoperatorid("0001A1100000000001M2");
				//单据类型
				Kcpg1.setPk_billtype("SW06");
		        //设置是否移交安装
				if( Kcpg1.getYjazrq() != null ){
					UFBoolean sfyjaz = new UFBoolean("Y");
					Kcpg1.setSfyjaz(sfyjaz);
				}
		...
		//中间表插入
		InvocationInfoProxy.getInstance().setUserDataSource(BZDS);
		String sqlwhereMill = "where   YiJiaoAnZhuangRiQi  is not null and baozhuangbianhao >= '2015000084' and baozhuangbianhao <='2015001624' ";
		List<FbyjazHVOC> listMill = (List<FbyjazHVOC>) NCLocator
				.getInstance()
				.lookup(IUAPQueryBS.class)
				.executeQuery(
						"select baozhuangbianhao RESERVE1,GCMC pk_jobbasfil, GongChengXingZhi gcxz, HeTongHao hth, SheJiTuHao sjth,YiJiaoAnZhuangRiQi yjazrq, BeiZhu remark from bz  " //勘察派工人员=勘察人员，勘察派工日期=勘察日期
						+ sqlwhereMill
						,new BeanListProcessor(FbyjazHVOC.class));    
		...
		List<FbyjazBVO> list2 = (List<FbyjazBVO>) NCLocator
				.getInstance()
				.lookup(IUAPQueryBS.class)
				.executeQuery(
						"select b.bzbh,b.lxr,b.lxdd, b.hm, b.pk_bzxx, a.pk_jobbasfil, a.gcxz, a.hth, a.sjth,a.yjazrq,a.remark " 
                        + "from   lcsw_fbyjaz_hCopy  a ,lcsw_bzxx  b " 
						+ sqlwhere2,
						new BeanListProcessor(FbyjazBVO.class));
		System.out.println("list2长度为："+list2.size());
		if (list2 != null) {
			for (FbyjazBVO Kcfksj2 : list2) {
				
			//	Kcfksj2.setPk_corp("0001");
				
				Kcfksj2.setDr(0);
				//设置关联主表id
				  String pk_fbyjaz = (String) NCLocator.getInstance().lookup(IUAPQueryBS.class).executeQuery("select a.pk_fbyjaz from lcsw_fbyjaz a where a.pk_jobbasfil ="
					+"'"+Kcfksj2.getPk_jobbasfil()+"' "
					 , new ColumnProcessor(null));
				   Kcfksj2.setPk_fbyjaz(pk_fbyjaz);
				//设置报装申请pk_bzsq
				   if( Kcfksj2.getPk_fbyjaz() != null){
